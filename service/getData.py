...
from db.queries.SQL_QUERIES import (sql_get_all_jobs, sql_get_job_count, sql_mark_job_as_applied, sql_get_all_applied_jobs,
                                    ...
                                    sql_get_available_job_count, sql_get_applied_jobs_count, sql_get_inactive_jobs_count)
...
def get_applied_jobs_count_from_db(job_category):
    conn = connect()
    cursor = conn.cursor()
    if job_category:
        cursor.execute(sql_get_applied_jobs_count(job_category), {"job_category": job_category})
    else:
        cursor.execute(sql_get_applied_jobs_count(job_category))
    rows = cursor.fetchall()
    conn.close()
    return rows[0][0]

def get_inactive_jobs_count_from_db(job_category):
    conn = connect()
    cursor = conn.cursor()
    if job_category:
        cursor.execute(sql_get_inactive_jobs_count(job_category), {"job_category": job_category})
    else:
        cursor.execute(sql_get_inactive_jobs_count(job_category))
    rows = cursor.fetchall()
    conn.close()
    return rows[0][0]
